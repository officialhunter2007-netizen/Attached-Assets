...
**Regression trap — GFM tables:** a per-line scrubber that blindly strips 3+ hyphens (the "runaway `---`" guard above) will also eat a markdown table's separator row (`|---|---|---|` → `|||||`), which breaks marked's GFM table-detection requirement and makes the WHOLE table fall back to a literal pipe-delimited paragraph. Any stray-markdown scrubber must be table-block-aware: detect a table row (line with ≥2 pipe-split cells, backtick-span-aware) followed by either a real separator row or another table row, and pass the whole detected block through untouched (never apply the stray-`-`/`#`/`>` strip inside it). As a bonus this same detection lets you deterministically repair the common LLM mistake of emitting a table with NO separator row at all (just header+data lines) by synthesizing one from the header's cell count — makes tables render correctly regardless of whether the model remembered GFM's separator-row requirement.
