#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# generate-v4-cpp-skill.sh — مُولِّد ملف تعليمات نُخبة v4 لمهارة «البرمجة بلغة C++»
#
# هذا السكريبت توأمٌ لـ generate-v4-python-skill.sh، لكنه مُخصَّص بالكامل لإنتاج
# ملف تعليمات عالي الجودة لمهارة *C++* داخل قسم المهارات (skills /
...
#   • أمثلة C++ من الحياة اليمنية (الصرافة، فواتير الكهرباء، السوق، الباصات…).
#   • توظيف الأدوات البصرية: cpp_trace لتتبّع التنفيذ، flowchart للخوارزميات،
#     و[[ANIM]] للعمليات الديناميكية (الحلقات، الاستدعاء الذاتي، الذاكرة، المؤشّرات).
#   • تدرّج حقيقي: المتغيّرات ⇒ التحكّم ⇒ الدوال ⇒ الهياكل ⇒ OOP ⇒ القوالب
#     ⇒ STL ⇒ الذاكرة الديناميكية ⇒ مشاريع صغيرة.
#
...
#   bash scripts/generate-v4-cpp-skill.sh all
#   (SPEC_SLUG و SPEC_NAME لهما قيمتان افتراضيتان لـ C++ — لا داعي لضبطهما.)
#
...
# قيم افتراضية مُخصَّصة لمهارة C++ في قسم المهارات (قابلة للتجاوز).
SPEC_SLUG="${SPEC_SLUG:-skill-cpp}"
SPEC_NAME="${SPEC_NAME:-البرمجة بلغة C++}"
SPEC_DESC="${SPEC_DESC:-مهارة عملية بحتة لإتقان لغة C++ من أول دقيقة — تبدأ بكتابة وتشغيل الكود فوراً دون أي مقدمة نظرية، وتنتهي ببناء مشاريع يمنية حقيقية. المنهج 3 مستويات مكثفة: تأسيس وتحكّم، هياكل ودوال، ثم OOP وخوارزميات ومشاريع متقدمة.}"
  ...
  while (( tries < max_tries )); do
    ...
    if [[ "$PROVIDER" == "vertex" ]]; then
      ...
      resp="$(curl -sS --max-time 600 -w $'\n%{http_code}' \
        ...
        -H "X-Title: Nukhba v4 C++ Skill Generator" \
...
# ── 2. التلقين الأساسي — هوية مؤلّف مناهج C++ عملية عالية الجودة ──────────
read -r -d '' SYS_BASE <<'EOF' || true
أنت خبير تدريس برمجة بلغة C++ ومُصمِّم مناهج عربي فائق الجودة، تبني مساراً عملياً بحتاً للطلاب اليمنيين. لا توجد محاضرات نظرية — كل شيء يبدأ من أول دقيقة بكتابة كود حقيقي وتشغيله. تُنتج ملف تعليمات v4.1 لمنصّة نُخبة (Nukhba) لمهارة «البرمجة بلغة C++» بـ 3 مستويات مكثفة عالية القدرات.
...
- المعلّم الذكي يملك أدوات بصرية: `cpp_trace` (تتبّع تنفيذ الكود سطراً سطراً مع المتغيّرات والذاكرة)، `flowchart` (الخوارزميات والقرارات)، و`[[ANIM]]` (رسوم متحرّكة للعمليات الديناميكية: الحلقات، الاستدعاء الذاتي recursion، حركة المؤشّرات في الذاكرة، إدارة الكومة heap، الفرز، تخصيص الذاكرة الديناميكية). صمّم المحتوى ليستدعي هذه الأدوات طبيعياً.

قواعد ذهبية مُلزِمة:
1. مخرجك دائماً مستند JSON واحد فقط — لا markdown، لا ``` ، لا شرح قبل أو بعد، لا تعليقات داخل JSON.
2. كل النصوص الشارحة بالعربية الفصحى المبسّطة (مفهومة لمبتدئ يمني)، عدا: أكواد C++، أسماء الدوال/الكلمات المفتاحية، slug/kind/bloom_focus. أيّ مصطلح تقني إنجليزي يُشرح بالعربية بجانبه أول مرة (مثال: «المؤشّر pointer: متغيّر يحفظ عنوان متغيّر آخر في الذاكرة»).
3. **C++ عملي لا حفظ**: كل درس يجب أن يحتوي كوداً حقيقياً قابلاً للتشغيل (داخل حقول النصّ مثل الأمثلة وحلول النموذج)، ومخرجاً متوقّعاً واضحاً. ممنوع درس بلا كود.
4. **أمثلة من الحياة اليمنية بكود C++**: حساب الباقي في السوق، تحويل العملة (ريال/دولار/سعودي) عند الصرّاف، جدول انقطاع الكهرباء، حساب أجرة الباص، فاتورة الدكان، أوزان البنّ والخضار، معدّل الطالب في الجامعة، مخزون الصيدلية. اجعل الكود يحلّ مشكلة يلمسها الطالب. **ممنوع منعاً باتاً ذكر القات في أي مثال** — استخدم بدائل محايدة ومقبولة: البنّ اليمني، التمور، العسل، البهارات، الأقمشة، المخلّلات، الأسماك.
...
6. **ثقافة تصحيح الأخطاء (debugging)**: في الأخطاء الشائعة لكل درس، أدرِج أخطاء C++ الواقعية التي يقع فيها المبتدئ (نسيان الفاصلة المنقوطة، استخدام المؤشّر بدون تهيئة، تجاوز حدود المصفوفة، خلط = بـ ==، تسرّب الذاكرة memory leak، خطأ الفهرسة off-by-one، نسيان #include، خلط cout/cin، استخدام متغيّر غير مُهيَّأ) مع التصحيح وكيف يُكتشف من رسالة الخطأ.
...
12b. **ممنوع استخدام نصوص قالبية أو عامة** مثل "خطأ شائع متوقّع في هذا الدرس" أو "التصحيح الصحيح" أو "عالج الخطأ بمثال موجَّه". كل خطأ common_mistake يجب أن يكون خطأً حقيقياً محدداً مرتبطاً بمحتوى الدرس (مثلاً: "نسيان الفاصلة المنقوطة بعد تعريف الهيكل struct"). إذا لم تستطع كتابة خطأ حقيقي، فاكتب درساً أقل بدلاً من ملء الباقي بنصوص قالبية.
13. **الدرس الأول عملي بحت — ابدأ الكود فوراً بلا أي نظرية**: لا تبدأ بـ «ما هي البرمجة» ولا «لماذا C++» ولا أي مقدمة تعريفية. الدرس الأول هو: اكتب `cout << "أهلاً";` وشغّله. ثم `cout << 5+3;`. ثم اقرأ اسم المستخدم بـ `cin` وأخرجه. كل مفهوم يُشرح أثناء كتابة الكود وتجربته، لا قبله. الطالب يكتب ويشغّل ويكتشف بنفسه من اللحظة الأولى. لا توجد فقرة «شرح نظري» منفصلة — الشرح كله داخل الكود وبعده مباشرة (سطر أو سطران).
14. **سؤال التحقّق النهائي (final_check_question)**: اجعله عملياً يطلب من الطالب كتابة كود أو توقّع مخرج كود C++. اكتب السؤال بصيغة مباشرة تصلح لأن تُعرَض مع أزرار اختيارات `[[ASK_OPTIONS: ...]]` حين يكون السؤال اختياراً من متعدّد. مثلاً: `[[ASK_OPTIONS: ما مخرج الكود cout << 2+3;؟ ||| 5 ||| 2+3 ||| خطأ ||| غير ذلك]]`. أما إذا طلب السؤال كتابة كود فعلي، فاتركه بصيغة مفتوحة بدون الوسم.

التدرّج العلمي المطلوب لمنهج C++ — 3 مستويات مكثفة عالية القدرات (التزِم بروحه):
- مستوى 1 (التأسيس والتحكّم): كتابة وتشغيل الكود من أول دقيقة، المتغيّرات والأنواع (int/float/double/char/bool/string)، الإدخال والإخراج (cin/cout)، العمليات الحسابية، الشروط if/else/switch، العوامل المنطقية، الحلقات for/while/do-while، break/continue، المصفوفات الأحادية والثنائية، الدوال وإعادة القيمة والوسائط، المراجع references. يغطي الأساسيات كاملةً في مستوى واحد مكثف.
- مستوى 2 (المؤشّرات والهياكل والـ OOP): المؤشّرات pointers وعلاقتها بالذاكرة، الذاكرة الديناميكية (new/delete)، الهياكل struct، الأصناف class والدوال المنتمية والمنشئ constructor والهادم destructor، الوراثة inheritance، تعدّد الأشكال polymorphism، الملفّات (ifstream/ofstream)، معالجة الاستثناءات try/catch.
- مستوى 3 (STL والخوارزميات والمشاريع): القوالب templates، حاوية STL (vector, map, set, queue, stack)، خوارزميات STL (sort, find, count)، التكرار iterators، الـ Lambda، خوارزميات بحث وفرز وتحليل التعقيد الزمني، تنظيم مشروع متعدّد الملفّات (header/source)، مشروع تطبيقي يمني متكامل (نظام إدارة دكان، حاسبة صرافة، متتبّع كهرباء)، أفضل ممارسات الكود النظيف (RAII، const correctness، التوثيق).

أسلوب الكتابة المطلوب:
- جمل واضحة، لا حشو. تعليمات تشغيلية للمعلم الذكي، لكن المحتوى التعليمي عميق لا سطحي.
- كل درس له: bridge_sentence (≥10 كلمات تربط بالدرس السابق بمثال كود محسوس)، 3-6 مفاهيم، 2-4 أخطاء C++ شائعة بعلاج محدّد، 1-3 أمثلة كود يمنية، سؤال تحقّق نهائي عملي (يطلب كتابة/توقّع كود)، معيار اكتمال جلسة.
- **عمق شرح المفهوم (إلزامي)**: حقل explanation لكل مفهوم فقرة كاملة (3-5 جمل، ≥40 كلمة) تتضمّن: (1) تعريف المفهوم البرمجي بوضوح، (2) لماذا يحتاجه المبرمج عملياً، (3) مقطع/مثال كود C++ قصير من حياة يمنية **داخل الشرح نفسه** (وليس فقط في حقل yemeni_examples المنفصل)، (4) الخطأ الشائع فيه وتصويبه. ممنوع شرح من سطر واحد. **حتى المفاهيم المجرّدة يجب أن تتضمّن سطر كود C++ واحداً على الأقل** يوضّح الفكرة عملياً — لا تكتفِ بالكلام النظري.
...
phase_skeleton() {
  ...
  local user
  user="$(cat <<EOF
أنشئ هيكل ملف تعليمات v4 لمهارة C++ التالية:
- slug: $SPEC_SLUG
- name: $SPEC_NAME
...
المطلوب JSON بهذا الشكل بالضبط:
{
  ...
  "specialty": {
    ...
    "language": "$SPEC_LANG",
    "region": "$SPEC_REGION",
    "target_persona": "وصف الطالب المستهدف: مبتدئ يمني (ثانوي/جامعي/باحث عمل) يريد إتقان C++ عملياً للوظائف والمشاريع، قد لا يملك خلفية برمجية (2-4 جمل)",
    "teacher_tone": "نبرة المعلّم: مشجّعة وعملية، تجعل الطالب يكتب كوداً من أول دقيقة ولا تخيفه من الأخطاء ولا من تعقيدات C++ (جملتان)",
    "allowed_viz_templates": ["cpp_trace", "flowchart", "tree_diagram", "bar_chart", "memory_diagram"],
    "allowed_tools": ["text", "code", "image"],
    "glossary": [
      {"term": "pointer مؤشّر", "definition": "تعريف عربي مبسّط بمثال يمني"}
    ]
  },
  "levels": [
    {
      ...
      "motivation_hook": "لماذا يهمّ الطالب هذا المستوى من C++ (جملتان)",
      "learning_objectives": [
          ...
          "units": [
            {
              ...
              "enables_unit_codes": [],
              "key_concepts": ["مفهوم C++ 1", "مفهوم 2", "مفهوم 3"],
              "exam_meta": {"pass_threshold_percent": 60},
              "lessons": [],
              "labs": []
            }
...
- key_concepts: 3-5 مفاهيم C++ موجزة لكل وحدة (بأسماء برمجية واضحة).
- التدرّج المنهجي لـ C++ 3 مستويات (التزِم به): مستوى1 التأسيس والتحكّم (متغيّرات/أنواع/cin-cout/شروط/حلقات/مصفوفات/دوال/مراجع)، مستوى2 المؤشّرات والهياكل والـOOP (pointers/new-delete/struct/class/وراثة/ملفّات/استثناءات)، مستوى3 STL والخوارزميات والمشاريع (templates/vector-map/lambda/فرز/مشاريع يمنية).
- bloom_focus: مستوى1 apply (يطبّق فوراً)، 2 analyze (يحلّل ويقارن)، 3 evaluate/create (يقيّم ويبني مشاريع).
- اجعل أسماء المراحل والوحدات تعكس مسار إتقان C++ حتى بناء مشروع يمني حقيقي.
...
# ── 4. مرحلة الوحدات: لكل وحدة في الهيكل → دروس C++ عملية + معامل ─────────
phase_units() {
  ...
  while IFS= read -r code; do
    ...
    local user
    user="$(cat <<EOF
هذه وحدة من ملف تعليمات مهارة «$SPEC_NAME» في نُخبة v4. املأ تفاصيلها الكاملة (دروس C++ عملية + معامل) فقط.
  ...
  "lessons": [
    {
      ...
      "bridge_sentence": "جملة افتتاحية ≥10 كلمات تربط بالكود السابق وتُمهّد بمثال C++ محسوس من حياة يمنية.",
      "prerequisite_lesson_codes": [],
      "enables_lesson_codes": [],
      "final_check_question": "سؤال تحقّق نهائي عملي: يطلب كتابة كود C++ قصير أو توقّع مخرج كود معيّن",
      "session_complete_criterion": "معيار يحكم اكتمال الجلسة (كتب الطالب كوداً صحيحاً يعمل وشرح منطقه بكلماته)",
      "yemeni_examples": ["مثال كود C++ يحلّ مشكلة يمنية محسوسة (مع وصف قصير للمخرج المتوقّع)"],
      ...
      "glossary": [{"term": "مصطلح C++", "definition": "تعريف عربي مبسّط بمثال"}],
      "concepts": [
        {"name": "...", "explanation": "فقرة كاملة (3-5 جمل، ≥40 كلمة): تعريف المفهوم البرمجي + لماذا يحتاجه المبرمج + مقطع كود C++ قصير من حياة يمنية + الخطأ الشائع وتصويبه — لا سطر واحد", "mastery_criterion": "متى نعتبر المفهوم مُتقَناً عملياً", "weight": 1}
      ],
      "common_mistakes": [
        {"mistake": "خطأ C++ شائع للمبتدئ كما يقع فيه فعلاً (مثل نسيان الفاصلة المنقوطة أو استخدام مؤشّر غير مُهيَّأ)", "correction": "التصحيح الموجز", "treatment": "كيف يعالجه المعلم ويعلّم الطالب قراءة رسالة الخطأ", "severity": "major"}
      ]
    }
  ],
  "labs": [
    {
      ...
      "scenario": "سرد قصصي يضع الطالب أمام مشكلة يمنية واقعية يحلّها ببرنامج C++ (مثل: بناء حاسبة فاتورة دكان، أو منظّم جدول كهرباء)",
      ...
      "questions": [
        ...
        {"question_index": 3, "kind": "application","prompt": "اطلب كتابة كود C++ فعلي", "rubric": "...", "solution_outline": "الكود النموذجي + المخرج", "points": 1},
        {"question_index": 4, "kind": "analysis",  "prompt": "اطلب تحليل/تصحيح كود C++ معطوب", "rubric": "...", "solution_outline": "...", "points": 1},
...
- كل معمل: 5 أسئلة، نوع واحد من كل من: diagnostic, decision, application, analysis, connection — بدون تكرار. واحد منها على الأقل يطلب كتابة كود C++ فعلي، وواحد يطلب تصحيح كود معطوب.
- كل درس: 3-6 مفاهيم، 2-4 أخطاء C++ شائعة، 1-3 أمثلة كود يمنية، solution_outline يحوي كوداً نموذجياً + مخرجاً.
...
- كل كود تكتبه C++ صحيح فعلاً (لو شُغِّل لعمل). تجنّب الكود الذي لا يعمل.
...
- common_mistakes: severity ∈ {minor, major, critical} حسب جسامة الخطأ. ركّز على أخطاء C++ الواقعية: نسيان الفاصلة المنقوطة، تجاوز حدود المصفوفة، استخدام مؤشّر غير مُهيَّأ، تسرّب الذاكرة، خلط =/==، نسيان #include، خلط نوع البيانات (int/double)، استخدام متغيّر بعد حذفه (dangling pointer).
- concepts: weight=1 افتراضياً، ارفعه إلى 2 أو 3 للمفهوم المحوري الذي لا يُسمح بالاجتياز بدونه (مثل: المؤشّرات، الكلاسات، القوالب).

أمثلة برمجية يمنية مطلوب توظيفها بكود C++: حاسبة الباقي في الدكان، تحويل العملة عند الصرّاف، جدول انقطاع الكهرباء، أجرة الباص، فاتورة الكافتيريا، أوزان البنّ/الخضار، معدّل الطالب، مخزون الصيدلية، قائمة مشتريات السوق.
...
# ── 5. مرحلة الامتحانات: بنوك C++ لكل (وحدة/مرحلة/مستوى) ──────────────────
gen_exam_bank() {
  ...
  local user
  user="$(cat <<EOF
أنشئ بنك أسئلة امتحان C++ لـ $scope "$code" في مهارة "$SPEC_NAME".
...
المطلوب JSON بهذا الشكل بالضبط:
{
  ...
  "variants": [
    [
      ...
      {"question_index": 2, "kind": "practical", "prompt": "اكتب دالة C++ تفعل كذا...", "rubric": "معايير قبول الكود (يعمل + يعطي المخرج الصحيح + أسلوب سليم)", "solution_outline": "الكود النموذجي + المخرج المتوقّع", "difficulty": 2, "points": 2, "time_limit_seconds": 240}
    ]
  ]
}
...
- أنواع أسئلة مسموحة: mcq | short_answer | practical.
  * mcq: 3-4 choices، correct_index سليم (0-based)، explanation واضح. فضّل أسئلة «ما مخرج هذا الكود؟» و«أين الخطأ في هذا الكود؟» و«ما الفرق بين pointer و reference؟».
  * practical: اطلب كتابة كود C++ فعلي، مع rubric + solution_outline (كود نموذجي + مخرج) إلزامي.
  * short_answer: rubric + solution_outline إلزامي.
- كل كود في الأسئلة C++ صحيح. احسب المخرج فعلياً قبل ضبط correct_index.
...
phase_placement() {
  ...
  local user
  user="$(cat <<EOF
أنشئ اختبار تحديد مستوى تكيّفي لمهارة C++ "$SPEC_NAME". الهدف: قياس مستوى الطالب في C++ بسرعة لاختيار المستوى الذي يبدأ منه.
...
- target_level_index: المستوى الذي يقيسه السؤال (1=أساسيات، 3=OOP/STL/مشاريع).
- difficulty داخل المستوى: 1 (تأسيسي) إلى 3 (المعيار المتوقّع لاجتيازه).
- معظم الأسئلة عملية: «ما مخرج هذا الكود؟» أو «أيّ كود يحقّق هذه النتيجة؟» أو «أين الخطأ؟» أو «ما الفرق بين pointer و reference؟».
...
phase_merge() {
  ...
  jq -n \
    ...
    --arg notes "الإصدار التلقائي بواسطة generate-v4-cpp-skill.sh — المهارة: $SPEC_NAME" \
    '($coreArr[0]) as $core
     | {
...
phase_validate() {
  ...
  if (( errors == 0 )); then
    ok "✅ اجتاز الفحص — ملف مهارة C++ جاهز للنشر في لوحة الإدارة."
    return 0
  fi
...
case "$cmd" in
  ...
  help|*)
    cat <<USAGE
الاستخدام: bash scripts/generate-v4-cpp-skill.sh <command>

مُولِّد ملف تعليمات مهارة «$SPEC_NAME» (قسم المهارات) — توأم مُخصَّص لـ C++
من generate-v4-python-skill.sh، بتعليمات أقوى وأعمق وعملية بالكامل.
...
الأوامر:
  ...
  units       2) املأ كل وحدة بدروس C++ عملية + معامل
...
ملاحظات:
  • الملف الكامل ≈ 357 نداء، يأخذ ساعات — شغّله في الخلفية:
      nohup bash scripts/generate-v4-cpp-skill.sh all > run-cpp.log 2>&1 &
      tail -f run-cpp.log
