...
- **Path engine** (`lib/v4-path-engine.ts`):
  ...
  - Adaptive placement (rebuilt — binary search over units): `generatePlacementQuestions` stratified-samples ≤22 units evenly across the whole curriculum and authors ~3 **practical, MCQ-only** questions per unit (Gemini Flash Lite, concurrency 8, code in fenced English/Latin blocks, Arabic stems); the pool must clear ≥13 questions across ≥6 distinct units or it throws and falls back. `evaluatePlacement` runs an adaptive **binary search over the numerically-ordered units** (best-of-3 per unit for fluke-resistance, tracking highest-pass / lowest-fail bounds) and places the student **conservatively at the first real unit AFTER the highest proven-mastered unit** (`start = realLo + 1`), soft-capped at 18 questions; each ask step returns difficulty + a confidence percentage. This replaced an earlier midpoint heuristic that consistently over-placed students by ~half the sampling gap (live-tested: a student knowing up to 2.4.1 was dropped at 2.4.5; conservative now lands at 2.4.2). `gradePlacementAnswer` grades MCQ by index comparison (free). On finalize, `buildPlacementProfile` distils the graded unit probes into a strengths/weaknesses snapshot persisted to `v4_student_paths.placement_profile` (jsonb — per-unit correct/wrong with Arabic unit names) so the AI teacher can personalize from the first lesson; the param is optional so a later from_zero re-setup can't wipe an earlier profile. The legacy level-only fallback (`evaluateLevelOnly` + `computeStartingLevel`) advances to the **first not-yet-mastered level** (= highest mastered + 1, correct ratio ≥ 0.66), clamped to the curriculum's level count by `computeUnlocked`.
...
- **Per-subject monthly wallet** (`student_gem_wallets`): one row per `(user_id, subject_id)` with a single `gems_balance` field — no daily cap, no midnight forfeit. `expires_at` is 30 days from purchase; the subscription ends at `expires_at` with **no grace window** (`V4_GRACE_DAYS = 0`, inclusive boundary) — the one-time 150-gem welcome gift is the student's trial.
...
- **Welcome gift** (global, one-time **150 gems**): granted ONCE per **student** (not per subject) via the platform-wide welcome pool (`student_welcome_gifts`), which the student distributes across up to 3 specialties (`lib/v4-welcome-gift.ts`). Allocated chunks land in the per-subject wallet through the same charge path as purchased gems. Replaces the retired per-subject +100 gift; the pool row + `welcome_gift_claimed` flag guard re-grants.
- **Carryover**: with grace at 0, a renewal made **at or before** `expires_at` preserves the leftover balance and audits it as `renewal_carryover`. A purchase after expiry starts fresh.
- **Atomic charge** (`chargeV4Ai`): `gems = floor(usd × 1000)` (1¢ = 10 gems). Idempotent on `requestId` via the same `gem_ledger(user_id, request_id)` unique index that powers `settleAiCharge`. Conditional UPDATE refuses to debit when balance is insufficient OR when past expiry (grace = 0).
- **Auto-refund** (`refundV4Ai`): reverses a debit by `requestId`. Idempotent (refund key = `${requestId}:refund`).
- **Expiry sweep**: `sweepV4ExpiredWallets` runs in the existing hourly `scheduled-jobs` tick. Zeroes any wallet whose `expires_at < now AND gems_balance > 0` (grace = 0) and writes a `monthly_expiry` audit row.
