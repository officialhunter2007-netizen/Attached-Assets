...
- **TryHackMe Integration**: Cybersecurity subjects (`uni-cybersecurity`, `skill-nmap`, `skill-wireshark`, `skill-linux`, `skill-windows`) integrate with TryHackMe platform. Features: account linking/unlinking, profile stats (rank, points, streak), badge display, curated room-to-lesson mappings (3-4 stages per subject), AI teacher emits `thm-room-card` HTML blocks with room recommendations, clickable room cards open TryHackMe directly. Panel accessible via red Shield button in chat dialog header. API routes: `/api/tryhackme/link`, `/unlink`, `/profile`, `/rooms/:subjectId`, `/mappings`. DB field: `tryhackmeUsername` on users table.
