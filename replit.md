...
- **TryHackMe Integration**: Cybersecurity subjects (`uni-cybersecurity`, `skill-nmap`, `skill-wireshark`, `skill-linux`, `skill-windows`) integrate with TryHackMe platform. Features: account linking/unlinking, profile stats (rank, points, streak), badge display, curated room-to-lesson mappings (3-4 stages per subject), AI teacher emits `thm-room-card` HTML blocks with room recommendations, clickable room cards open TryHackMe directly. Panel accessible via green pirate flag button in chat dialog header. API routes: `/api/tryhackme/link`, `/unlink`, `/profile`, `/rooms/:subjectId`, `/mappings`. DB field: `tryhackmeUsername` on users table.
- **Cybersecurity Lab (مختبر الأمن السيبراني)**: Interactive hands-on lab for cybersecurity subjects — 7 tools: **Linux Terminal Simulator** (full filesystem with /etc/passwd, /var/log, home dirs, commands: ls/cd/cat/grep/find/ping/ifconfig/netstat/nslookup/ps, tab completion, command history), **Port Scanner** (Nmap-like simulation with 4 target servers, SYN/Connect/UDP/Version scans, risk assessment, security alerts), **Encryption/Decryption Lab** (Caesar, Vigenère, Base64, ROT13, XOR, Atbash ciphers with encrypt/decrypt modes), **Hash Cracker** (SHA-256/MD5 hash generation + dictionary attack with 35 common passwords, animated cracking log), **SQL Injection Lab** (3 difficulty levels: no protection/partial filter/parameterized queries, with hints and vulnerability explanations), **XSS Lab** (3 levels: no filtering/partial script filter/output encoding, rendered preview showing attack effects), **Packet Analyzer** (Wireshark-like interface with 4 capture scenarios: normal traffic, SSH brute force, port scan, DNS exfiltration, packet detail inspector). All tools share-with-teacher enabled. Located at `artifacts/nukhba/src/components/cyber-lab/`.
...
- `GET/POST /api/tryhackme/*` — TryHackMe integration (link, unlink, profile, rooms, mappings)
