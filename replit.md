...
- **Universal AI-Built Practice Environments (بيئات تطبيقية)**: Replaced the old hard-coded Cybersecurity Lab. Available across **all** subjects — not limited to cyber. Three entry points: (1) permanent floating amber button "ابنِ بيئة تطبيقية" on every subject page, (2) AI teacher inline button via `[[CREATE_LAB_ENV: <topic>|<difficulty>]]` tag, (3) free-text command (e.g. "ابنِ لي بيئة تطبيقية عن…"). Flow: teacher proposes 3–5 multiple-choice intake questions (specialization, depth, focus), then a specialized builder model generates a real interactive UI as a `DynComponent` tree. **New universal component types** (in `dynamic-env/types.ts` + `component-renderer.tsx`): `webApp` (sandboxed iframe with restrictive `sandbox="allow-scripts allow-forms"` — never `allow-same-origin` — and per-render nonce-validated `postMessage` bridge via injected `window.envEmit(type, data)`), `packetCapture` (Wireshark-style packet list + layered drill-down), `terminal` (read-only console viewer), `fileSystemExplorer` (tree + content pane + optional download), `browser` (multi-tab sandboxed mini-browser), `networkDiagram` (SVG topology with auto circular layout when no x/y given), `logViewer` (filterable structured logs by level + text). Backend route `/api/ai/lab/build-env` (Claude Sonnet 4.6, `max_tokens=16384`, parse-failure fallback env). Assist endpoint `/api/ai/lab/assist` enriched with `worldStateSummary`, `lastMutation`, `currentTask`, `consoleOutput` so the helper sees live env state. The legacy Food/Accounting/YemenSoft scenario overlays remain (`LabKind = "food" | "accounting" | "yemensoft"`); cyber/nmap/wireshark `LabKind` removed. Old code deleted: `cyber-lab/` directory, `/ai/cyber/exec`, `/ai/cyber/web-page`, `/ai/cyber/create-env`, `normalizeAIEnv`, `randomMAC`, `generateVulnerablePageHTML`, `supportsLabEnv` gate.
