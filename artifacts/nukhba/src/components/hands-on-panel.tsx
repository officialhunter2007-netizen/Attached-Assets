 ...
 *
 * NEW: floating "مساعدة" button opens a Socratic AI mini-chat — free guidance
 * that hints without giving the answer, context-aware of the current task.
 */
import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import { Loader2, X, Wrench, MessageCircle, Send, ChevronDown } from "lucide-react";
...
export function HandsOnPanel({
  ...
  const [helpHistory, setHelpHistory] = useState<HelpMsg[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    setHelpOpen(false); setHelpHistory([]); setHelpInput("");
    try {
  ...
  // Scroll help chat to bottom whenever messages change
  useEffect(() => {
    if (helpOpen) helpEndRef.current?.scrollIntoView({ behavior: "smooth" });
  }, [helpHistory, helpOpen]);

  // Focus input when chat opens
  useEffect(() => {
    if (helpOpen) setTimeout(() => helpInputRef.current?.focus(), 120);
  }, [helpOpen]);
  ...
  async function sendHelp() {
    ...
    setHelpInput("");
    const newMsg: HelpMsg = { role: "user", content: q };
    const updatedHistory = [...helpHistory, newMsg];
    ...
    try {
      ...
      const answer = typeof data?.answer === "string" && data.answer.trim()
        ? data.answer.trim()
        : "تعذّر الاتصال — جرّب مرة ثانية.";
      setHelpHistory([...updatedHistory, { role: "assistant", content: answer }]);
    } catch {
      setHelpHistory([...updatedHistory, { role: "assistant", content: "تعذّر الاتصال." }]);
    } finally {
  ...
  const showHelp = phase === "answer" || phase === "grading";
  ...
  return (
    ...
    >
      <div
        className="relative w-full sm:max-w-lg max-h-[92dvh] overflow-y-auto bg-[#11142a] border border-amber-400/30 rounded-t-3xl sm:rounded-3xl shadow-2xl flex flex-col"
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          {/* spacer so content isn't hidden behind the sticky help button */}
          {showHelp && <div className="h-2" />}
        </div>

        {/* ── Floating help button (sticky bottom of panel) ── */}
        {showHelp && !helpOpen && (
          <div className="sticky bottom-0 px-5 pb-4 pt-2 bg-gradient-to-t from-[#11142a] to-transparent pointer-events-none">
            <button
              onClick={() => setHelpOpen(true)}
              className="pointer-events-auto mr-auto flex items-center gap-2 px-4 py-2.5 rounded-2xl bg-gradient-to-l from-purple-600 to-purple-500 text-white text-sm font-bold shadow-lg shadow-purple-900/40 hover:from-purple-500 hover:to-purple-400 transition-all active:scale-95"
              title="اطلب مساعدة من الذكاء الاصطناعي"
            >
        ...
        {/* ── Help chat drawer ── */}
        {showHelp && helpOpen && (
          <div className="flex flex-col border-t border-purple-500/30 bg-[#0e1024]" style={{ maxHeight: "55%" }}>
            {/* Chat header */}
            <div className="flex items-center gap-2 px-4 py-2.5 border-b border-white/5">
              <MessageCircle className="w-4 h-4 text-purple-400 shrink-0" />
              <span className="text-xs font-bold text-purple-200 flex-1">مساعد الذكاء الاصطناعي</span>
              <span className="text-[10px] text-white/40">مجاني · يوجّه بدون إعطاء الحل</span>
              <button
            ...
            <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3 min-h-0">
              {helpHistory.length === 0 && (
                <div className="text-center text-xs text-white/30 py-4 leading-relaxed">
                  اسألني أي شيء يخص المهمة — سأوجّهك بسؤال يقودك للإجابة 🧠
                </div>
              )}
              {helpHistory.map((m, i) => (
                <div
                  ...
                  <div
                    className={`max-w-[82%] rounded-2xl px-3.5 py-2.5 text-xs leading-relaxed ${
                      m.role === "user"
                        ...
                        : "bg-purple-600/25 border border-purple-500/30 text-purple-100 rounded-tl-sm"
                    }`}
                  >
                    {m.content}
                  </div>
              ...
              {helpLoading && (
                <div className="flex justify-end">
                  <div className="bg-purple-600/20 border border-purple-500/25 rounded-2xl rounded-tl-sm px-4 py-3 flex gap-1.5">
                    {[0, 1, 2].map((i) => (
                      <span
                        key={i}
            ...
            {/* Input */}
            <div className="flex gap-2 px-3 pb-3 pt-2 border-t border-white/5">
              <textarea
                ref={helpInputRef}
                value={helpInput}
                onChange={(e) => setHelpInput(e.target.value)}
                onKeyDown={(e) => {
                  if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); void sendHelp(); }
                }}
                disabled={helpLoading}
                rows={2}
                placeholder="اكتب سؤالك…"
                className="flex-1 resize-none bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-xs text-white placeholder:text-white/30 focus:outline-none focus:border-purple-400/50 disabled:opacity-50 leading-relaxed"
              />
              <button
                onClick={() => void sendHelp()}
                disabled={helpLoading || !helpInput.trim()}
                className="self-end p-2.5 rounded-xl bg-purple-600 text-white disabled:opacity-40 hover:bg-purple-500 active:scale-95 transition-all"
                aria-label="إرسال"
              >
