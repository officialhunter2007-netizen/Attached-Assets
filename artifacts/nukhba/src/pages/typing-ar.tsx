...
import { Lock, Star, ChevronDown, ChevronUp, Keyboard, ArrowLeft } from "lucide-react";
import { sections, type Lesson, type Section } from "@/lib/typing-curriculum-ar";
...
function LessonCard({
  ...
  return (
    <Link href={locked ? "#" : `/typing-ar/lesson/${lesson.id}`}>
      <motion.div
          ...
          {done && (
            <div className="flex items-center gap-2 mt-0.5">
              ...
              <span className="text-[10px]" style={{ color: "rgba(255,255,255,0.3)" }}>
                {progress?.wpm} ك/د · {progress?.accuracy}% دقة
              </span>
            </div>
          )}
        </div>
...
function SectionPanel({
  ...
  return (
    <div
      ...
      style={{ background: section.bgColor, border: `1px solid ${section.color}30` }}
    >
      <button
        dir="rtl"
      ...
      >
        ...
        <div className="flex-1 min-w-0 text-right">
          <div className="flex items-center gap-2">
            ...
            >
              القسم {section.index + 1}
            </span>
          </div>
          <div className="text-xs mt-0.5 text-right" style={{ color: "rgba(255,255,255,0.45)" }}>
            {section.subtitle}
          </div>
...
export default function TypingAr() {
  ...
  function getUnlockedUpTo(): number {
    ...
    let last = allLessons[0]?.id ?? 10001;
  ...
  const unlockedUpTo = loading ? 10001 : getUnlockedUpTo();
      ...
      <div className="min-h-screen" style={{ direction: "rtl" }}>
        <div className="relative overflow-hidden py-10 px-4">
          <div
            ...
            style={{ background: "radial-gradient(ellipse at 50% 0%, rgba(16,185,129,0.10) 0%, transparent 70%)" }}
          />
          <div className="relative max-w-2xl mx-auto">
            ...
            <div className="text-center">
              ...
              >
                ...
                تدريب الكتابة العربية
              </div>
              <h1 className="text-3xl md:text-4xl font-black text-white mb-2">
                تعلم الكتابة السريعة بالعربية
              </h1>
              <p className="text-sm text-white/40 mb-1">
                {totalLessons} درساً · 14 قسماً · من الصف الرئيسي إلى إتقان السرعة
              </p>
              <p className="text-xs text-white/25 mb-6">
                تدريبات منهجية لبناء السرعة والدقة بدون النظر إلى لوحة المفاتيح
              </p>

              <div className="max-w-xs mx-auto mb-3">
                <div className="flex justify-between text-xs mb-1.5" style={{ color: "rgba(255,255,255,0.4)" }}>
                  <span className="font-bold" style={{ color: "#10B981" }}>{overallPct}%</span>
              ...
              {firstIncomplete && (
                <Link href={`/typing-ar/lesson/${firstIncomplete.id}`}>
                  <motion.button
                    whileTap={{ scale: 0.97 }}
                    className="mt-4 px-6 py-3 rounded-2xl text-sm font-bold text-white"
                    style={{ background: "linear-gradient(135deg, #10B981, #059669)" }}
                  >
                    {totalCompleted === 0 ? "ابدأ التدريب" : "واصل من حيث توقفت"} ←
                  </motion.button>
                </Link>
              )}
            </div>
        ...
        <div className="max-w-2xl mx-auto px-4 pb-12 space-y-3">
          {loading ? (
            ...
            sections.map((section, si) => {
              const firstLessonId = section.lessons[0]?.id ?? 10001;
