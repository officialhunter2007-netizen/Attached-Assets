/**
 * v4-lab.tsx — Lab scenario + 5 typed questions, sequential UX (task #7).
 *
 * Flow (spec §13.1 — "Haiku يُقيّم كل إجابة فوراً بصواب/خطأ ويوضّح"):
 *   1. GET /api/v4/lab/:slug/:labCode → scenario + 5 questions.
 *   2. Student answers ONE question at a time. On submit:
 *      POST /api/v4/lab/:slug/:labCode/evaluate → per-question Haiku
 *      verdict + explanation displayed inline.
 *   3. After seeing feedback, student presses "next" to reveal Q+1.
 *   4. After Q5, auto-call POST .../submit which RE-EVALUATES on the
 *      server (no client-trust) and persists the final attempt.
 *   5. Result screen: passed (≥60) celebrates; failed offers retry.
 *
 * Question kinds (fixed 5 in order): diagnostic, decision, application,
 * analysis, connection.
 */
...
type LabPayload = {
  ...
  prior: { score: number; passed: boolean; attempts: number; evaluatorLog: EvalRow[] } | null;
...
export default function V4Lab() {
  ...
  // Sequential UX state.
  const [cursor, setCursor] = useState(0);                       // current Q index (0..4)
  ...
  useEffect(() => {
    if (!slug || !labCode) return;
    let cancelled = false;
    (async () => {
      try {
        const r = await fetch(`/api/v4/lab/${encodeURIComponent(slug)}/${encodeURIComponent(labCode)}`, { credentials: "include" });
        if (!r.ok) throw new Error(`http_${r.status}`);
        const d: LabPayload = await r.json();
        if (cancelled) return;
        setData(d);
  ...
  async function evaluateCurrent() {
    ...
    setEvaluating(true);
    try {
      const r = await fetch(`/api/v4/lab/${encodeURIComponent(slug)}/${encodeURIComponent(labCode)}/evaluate`, {
        method: "POST",
        credentials: "include",
  ...
  async function advance() {
    ...
    setFinalizing(true);
    try {
      const r = await fetch(`/api/v4/lab/${encodeURIComponent(slug)}/${encodeURIComponent(labCode)}/submit`, {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json", "X-Nukhba-Csrf": "1" },
  ...
  return (
    <div className="min-h-[100dvh] bg-background text-white pb-24" style={{ direction: "rtl", fontFamily: "Tajawal, Cairo, sans-serif" }}>
      ...
      <div className="max-w-2xl mx-auto px-4 pt-6 space-y-5">
        ...
        {result ? (
          <ResultPanel result={result} onRetry={retry} onBackToMap={() => navigate(`/specialty/${encodeURIComponent(slug)}/map`)} />
        ) : (
          <>
            ...
            <div className={`rounded-3xl border bg-gradient-to-br p-5 ${KIND_COLOR[currentQ.kind] ?? "from-white/5 to-white/0 border-white/10"}`}>
              ...
              <p className="text-white/95 leading-relaxed mb-3 text-base">{currentQ.prompt}</p>
              <textarea
