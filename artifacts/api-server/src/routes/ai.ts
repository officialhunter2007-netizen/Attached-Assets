...
**🎨 معايير الجودة الإلزامية للبيئات (الفرق بين بيئة عادية وبيئة فاخرة):**
A. **briefing** يجب أن يكون مهنياً ومنظماً: سطر للسياق، سطر للدور الذي يلعبه الطالب، سطر للهدف النهائي. تجنّب الجمل الطويلة المتكررة.
B. **objectives** قائمة من 2-4 أهداف محددة وقابلة للقياس (تبدأ بفعل: احسب، أنشئ، حلّل، صحّح…) — لا أهداف عامة مثل "تعلّم المحاسبة".
C. **tasks** قائمة مرتّبة منطقياً 4-7 مهام، كل مهمة تبدأ بفعل أمر واضح وتشير إلى \`targetScreen\` صحيح. **رتّبها كمسار تعلّمي تصاعدي** (سهل → متقدّم) لا كقائمة عشوائية.
D. **screens** مجمّعة حسب الوظيفة (مثلاً: "الإعداد" / "التنفيذ" / "التحقق" / "النتائج")، لا شاشات متناثرة. كل شاشة لها \`icon\` ووظيفة محددة في عنوانها.
E. **successCriteria** قائمة 2-4 معايير يستطيع الطالب التحقق منها بنفسه (مثل: "ميزان المراجعة متوازن"، "كل الفواتير مرحّلة"، "الكميات في المخزون منطقية") — لا معايير ضبابية.
F. أوّل شاشة يجب أن تحتوي على \`alert\` ترحيبي أو \`richDocument\` يشرح الموقف بوضوح ويوجّه الطالب لأول مهمة.`;
