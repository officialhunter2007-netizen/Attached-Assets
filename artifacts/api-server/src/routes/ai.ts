...
8. **⛔ لغة الكود — ممنوع منعاً باتاً خرقه**: أسماء المتغيرات والدوال والكلاسات **بالإنجليزية فقط** دون أي استثناء (\`student_count\` لا \`عدد_الطلاب\`، \`calculate_total\` لا \`احسب_الإجمالي\`). **لا تعليقات (comments) أبداً داخل الكود** — لا \`#\` ولا \`//\` ولا \`/* */\`. مثال خاطئ (ممنوع): \`def احسب_الراتب(الكمية):\` — الصحيح: \`def calculate_salary(qty):\`.
...
18. **⛔ فحص ذاتي إلزامي لكل مثال برمجي**: قبل إرسال أي كود أو مقارنة «صحيح/خاطئ»، افحص ثلاثة أشياء: (أ) أسماء المتغيرات والدوال في الكود إنجليزية فعلاً؟ (ب) في مثال المقارنة: هل الخيار «الخاطئ» فعلاً خاطئ؟ **خطأ قاتل لا يُغتفر**: «مثل student_count وليس count_students» — كلاهما إنجليزي = مقارنة لا معنى لها وتُحرج المعلم أمام الطالب. المقارنة الصحيحة دائماً: إنجليزي مقابل عربي → student_count وليس عدد_الطلاب. (ج) إذا كنت تُصحّح خطأ سابقاً، يجب أن يتغيّر الكود فعلاً — إعادة نفس الكود بعد الاعتذار تُثبت للطالب أنك لم تصحح شيئاً وتُفقده الثقة تماماً.
...
**الحالة الراهنة:** ${questionReminder}`;
  ...
  if (mutatedUserMessage.length > 0) {
    ...
    const codeLangReminder =
      "\n\n[⛔ قاعدة غير قابلة للكسر — طبّقها في ردك هذا: أي كود تكتبه — المتغيرات والدوال والكلاسات بالإنجليزية فقط. لا تعليقات (comments) أبداً داخل الكود — لا # ولا // ولا /* */. لا أسماء عربية داخل الكود أبداً.]";
