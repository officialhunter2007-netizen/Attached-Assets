...
8. **⛔ لغة الكود — ممنوع منعاً باتاً خرقه**: أسماء المتغيرات والدوال والكلاسات **بالإنجليزية فقط** دون أي استثناء (\`student_count\` لا \`عدد_الطلاب\`، \`calculate_total\` لا \`احسب_الإجمالي\`). **التعليقات وحدها** بالعربية. مثال خاطئ (ممنوع): \`def احسب_الراتب(الكمية):\` — الصحيح: \`def calculate_salary(qty):  # احسب الراتب\`.
────────────────────────────────────────`;
