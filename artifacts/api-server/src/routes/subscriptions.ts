...
// Sane upper bound for any single plan price (in YER). Real plans are in the
// low tens of thousands of YER; anything above this is almost certainly a
// data-entry error (extra zeros) and would mint an absurd gem balance through
// the 50/50 split, so we refuse it outright.
const MAX_PLAN_PRICE_YER = 5_000_000;
...
router.post("/subscriptions/request", async (req, res): Promise<void> => {
  ...
  // Upper-bound guard: a misconfigured / fat-fingered plan_prices row (e.g. an
  // extra few zeros) would otherwise flow straight into the 50/50 split and
  // mint an absurd gem balance. Refuse clearly so the admin fixes pricing
  // instead of silently granting a fortune in gems.
  if (basePrice > MAX_PLAN_PRICE_YER) {
    logger.error(
      { planType: parsed.data.planType, region: parsed.data.region, basePrice, max: MAX_PLAN_PRICE_YER },
      "subscriptions/request: plan_prices row exceeds the sane upper bound — refusing request",
    );
    res.status(500).json({ error: "سعر هذه الباقة غير صالح. يرجى التواصل مع الدعم." });
    return;
  }
...
router.post("/subscriptions/activate", async (req, res): Promise<void> => {
  ...
  // NOTE: the card is NOT claimed here. The conditional claim (WHERE
  // isUsed=false) is done INSIDE the grant transaction below, so a failure in
  // the legacy grant or v4 funding rolls the claim back atomically — there is
  // no window where a card is marked used but the student got nothing. All the
  // resolution work below is read-only.
  ...
  if (!resolvedRegion || resolvedPaidYer <= 0) {
    // Hard refuse: nothing has been claimed yet (the claim now lives inside the
    // transaction below), so the card stays reusable automatically. Leaving
    // region null / price 0 in the live subscription would silently weaken the
  ...
  // Card claim, legacy grant, AND v4 wallet funding all run in ONE transaction.
  // The conditional claim (WHERE isUsed=false) gives concurrency safety, and
  // because it is inside the tx, any failure in the grant or v4 funding rolls
  // the claim back atomically — there is never a window where the card is
  // marked used but the student got nothing (no compensating release needed).
  let sub: Awaited<ReturnType<typeof grantSubjectSubscriptionInTx>>;
  try {
    const out = await db.transaction(async (tx) => {
      ...
      const granted = await grantSubjectSubscriptionInTx(tx, {
        userId,
      ...
      await purchaseV4GemsTx(tx, {
        userId,
        subjectId: granted.subjectId,
        subjectName: granted.subjectName ?? null,
        paidPriceYer: resolvedPaidYer,
        region: resolvedRegion,
        subscriptionRequestId: card.subscriptionRequestId ?? null,
        activationCode: code,
    ...
    // The transaction rolled back (claim included) — the card is still
    // reusable, so the student can simply retry.
    logger.error(
      { err: err?.message, userId, subjectId: card.subjectId, code },
      "subscriptions/activate: grant + v4 funding failed — transaction rolled back, card reusable",
    );
    res.status(500).json({
      success: false,
      message: "تعذّر تفعيل الكود الآن. يرجى المحاولة مرة أخرى بعد قليل.",
    });
    return;
  }
...
router.post("/admin/subscription-requests/:id/approve", requireSameOriginCsrf, async (req, res): Promise<void> => {
  const userId = getUserId(req);
  if (!userId) {
    res.status(401).json({ error: "Unauthorized" });
    return;
  ...
  try {
    const { card, subscription, v4Result } = await db.transaction(async (tx) => {
      ...
      // ── v4 monthly wallet: funded INSIDE the same transaction ──────────
      // The FE reads ONLY the v4 wallet, so this IS the balance the student
      // sees. Funding it must be atomic with the approval: if it fails the
      // whole approval rolls back and the admin retries — the student is
      // never left "paid but with an empty wallet".
      const v4Result = await purchaseV4GemsTx(tx, {
        userId: request.userId,
        subjectId: insertedSub.subjectId,
        subjectName: insertedSub.subjectName ?? null,
        paidPriceYer: request.finalPrice ?? request.basePrice ?? 0,
        region: request.region,
        subscriptionRequestId: id,
        activationCode: code,
      ...
      return { card: insertedCard, subscription: insertedSub, v4Result };
    });
    ...
    // back and we'd be in the catch below — so reaching here means the student
    // wallet is funded and matches what they paid for.
    logger.info(
      {
        userId: request.userId,
        subjectId: subscription.subjectId,
        walletId: v4Result.walletId,
        balanceAfter: v4Result.balanceAfter,
        carriedOver: v4Result.carriedOver,
      },
      "v4-wallet: purchase committed atomically with approve",
    );
...
router.post("/admin/subscription-requests/:id/reject", requireSameOriginCsrf, async (req, res): Promise<void> => {
  const userId = getUserId(req);
  if (!userId) {
    res.status(401).json({ error: "Unauthorized" });
    return;
...
router.post("/admin/subscription-requests/:id/incomplete", requireSameOriginCsrf, async (req, res): Promise<void> => {
  const userId = getUserId(req);
  if (!userId) {
    res.status(401).json({ error: "Unauthorized" });
    return;
...
router.post("/admin/cards/create", requireSameOriginCsrf, async (req, res): Promise<void> => {
  const userId = getUserId(req);
  if (!userId) {
    res.status(401).json({ error: "Unauthorized" });
    return;
...
router.patch("/admin/plan-prices", requireSameOriginCsrf, async (req, res): Promise<void> => {
  const userId = getUserId(req);
  if (!userId) { res.status(401).json({ error: "Unauthorized" }); return; }
  const user = await getUser(userId);
  if (user?.role !== "admin") { res.status(403).json({ error: "Forbidden" }); return; }
...
router.patch("/admin/exchange-rates", requireSameOriginCsrf, async (req, res): Promise<void> => {
  const userId = getUserId(req);
  if (!userId) { res.status(401).json({ error: "Unauthorized" }); return; }
  const user = await getUser(userId);
  if (user?.role !== "admin") { res.status(403).json({ error: "Forbidden" }); return; }
...
router.post("/admin/users/:id/cancel-subscription", requireSameOriginCsrf, async (req, res): Promise<void> => {
...
router.delete("/admin/users/:userId/subject-subscriptions/:subId", requireSameOriginCsrf, async (req, res): Promise<void> => {
  const adminId = getUserId(req);
  if (!adminId) { res.status(401).json({ error: "Unauthorized" }); return; }
  const admin = await getUser(adminId);
  if (admin?.role !== "admin") { res.status(403).json({ error: "Forbidden" }); return; }
...
router.delete("/admin/users/:userId/subject-first-lesson/:subjectId", requireSameOriginCsrf, async (req, res): Promise<void> => {
  const adminId = getUserId(req);
  if (!adminId) { res.status(401).json({ error: "Unauthorized" }); return; }
  const admin = await getUser(adminId);
  if (admin?.role !== "admin") { res.status(403).json({ error: "Forbidden" }); return; }
...
router.delete("/admin/revoke-subject-subscription/:subId", requireSameOriginCsrf, async (req, res): Promise<void> => {
  const adminId = getUserId(req);
  if (!adminId) { res.status(401).json({ error: "Unauthorized" }); return; }
  const admin = await getUser(adminId);
  if (admin?.role !== "admin") { res.status(403).json({ error: "Forbidden" }); return; }
...
router.patch("/admin/subject-subscriptions/:subId/extend", requireSameOriginCsrf, async (req, res): Promise<void> => {
  const adminId = getUserId(req);
  if (!adminId) { res.status(401).json({ error: "Unauthorized" }); return; }
  const admin = await getUser(adminId);
  if (admin?.role !== "admin") { res.status(403).json({ error: "Forbidden" }); return; }
...
router.post("/admin/subject-subscriptions/:subId/refund-gems", requireSameOriginCsrf, async (req, res): Promise<void> => {
  const adminId = getUserId(req);
  if (!adminId) { res.status(401).json({ error: "Unauthorized" }); return; }
  const admin = await getUser(adminId);
  if (admin?.role !== "admin") { res.status(403).json({ error: "Forbidden" }); return; }
...
router.post("/admin/discount-codes", requireSameOriginCsrf, async (req, res): Promise<void> => {
  const adminId = getUserId(req);
  if (!adminId) { res.status(401).json({ error: "Unauthorized" }); return; }
  const admin = await getUser(adminId);
  if (admin?.role !== "admin") { res.status(403).json({ error: "Forbidden" }); return; }
...
router.patch("/admin/discount-codes/:id", requireSameOriginCsrf, async (req, res): Promise<void> => {
  const adminId = getUserId(req);
  if (!adminId) { res.status(401).json({ error: "Unauthorized" }); return; }
  const admin = await getUser(adminId);
  if (admin?.role !== "admin") { res.status(403).json({ error: "Forbidden" }); return; }
...
router.post("/admin/users/:id/grant-gems", requireSameOriginCsrf, async (req, res): Promise<void> => {
  const adminId = getUserId(req);
  if (!adminId) { res.status(401).json({ error: "Unauthorized" }); return; }
  const admin = await getUser(adminId);
  if (admin?.role !== "admin") { res.status(403).json({ error: "Forbidden" }); return; }
