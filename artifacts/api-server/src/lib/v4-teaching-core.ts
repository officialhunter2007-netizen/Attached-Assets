...
export function buildPersonaLayer(
  ...
  return [
    ...
    "- **⚠️ السؤال داخل الوسم وليس خارجه**: الجزء الأول قبل أول `|||` هو السؤال نفسه. لا تكتب السؤال في نص الرسالة ثم تضع الخيارات فقط في الوسم — هذا خطأ شائع. نص رسالتك يكون الشرح أو السياق فقط، ثم الوسم يحتوي على **السؤال متبوعاً بالخيارات**.",
